...
import { InvoiceFluvo11Branding } from "@/features/invoices/templates/invoice_fluvo_11/invoice-fluvo-11-branding";
import { InvoiceFluvo11Signature } from "@/features/invoices/templates/invoice_fluvo_11/invoice-fluvo-11-signature";
import styles from "@/features/invoices/templates/invoice_fluvo_11/invoice-fluvo-11.module.css";
import { cn } from "@/lib/utils/cn";

const WINE = "#6b2d2d";
const ROW_TINT = "bg-rose-50/70";
...
const FIELD =
  "w-full min-w-0 border-0 border-b border-transparent bg-transparent py-0.5 text-sm text-zinc-800 placeholder:text-zinc-400 focus:border-rose-600 focus:outline-none";
const FIELD_RIGHT = `${FIELD} text-right tabular-nums`;

export function InvoiceFluvo11Template() {
        ...
        <section className="mt-10 space-y-4">
          <div className="overflow-x-auto rounded-sm">
            <table className="w-full min-w-[560px] border-collapse text-sm">
              <thead>
                <tr className="text-white" style={{ backgroundColor: WINE }}>
              ...
              <tbody>
                {lines.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      <td className="border border-rose-200/80 p-2 align-top">
                        <input
                          className={FIELD}
                      ...
                      <td className="border border-rose-200/80 p-2 align-top text-center">
                        <input
                      ...
                      <td className="border border-rose-200/80 px-2 py-2 text-right align-top tabular-nums font-medium text-zinc-900">
                        {formatBdt(amt)}
                      </td>
                    </tr>
                  );
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={addLine}
              className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50/80 px-3 py-1.5 text-xs font-medium text-rose-900 hover:bg-rose-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-600"
            >
              <Plus className="size-3.5 text-rose-800" aria-hidden />
              Add line
            </button>
            {lines.length > 1 ? (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-zinc-600 hover:text-rose-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-600"
              >
          ...
          <div className="w-full max-w-sm space-y-0 text-sm">
            ...
            <div className="flex items-center justify-between gap-6 border-b border-zinc-200 py-2.5">
              <span className="font-bold text-zinc-900">
                Tax (
                <input
                  className="inline-block w-9 border-0 border-b border-transparent bg-transparent text-right font-bold focus:border-rose-600 focus:outline-none"
        ...
        <section className="mt-14 grid gap-12 lg:grid-cols-2 lg:items-start">
          <div className="flex min-w-0 flex-col gap-10">
            ...
            <footer className="border-t border-zinc-200 pt-6 text-sm">
              ...
              <textarea
                className="mt-2 min-h-[5rem] w-full resize-y rounded-sm border-0 bg-transparent text-xs leading-relaxed text-zinc-600 focus:outline-none focus:ring-2 focus:ring-rose-500/25"
                value={terms}

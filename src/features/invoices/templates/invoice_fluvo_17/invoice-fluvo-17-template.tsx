...
import { InvoiceFluvo17Branding } from "@/features/invoices/templates/invoice_fluvo_17/invoice-fluvo-17-branding";
import { InvoiceFluvo17Signature } from "@/features/invoices/templates/invoice_fluvo_17/invoice-fluvo-17-signature";
import { InvoiceFluvo17HeaderSwoosh } from "@/features/invoices/templates/invoice_fluvo_17/invoice-fluvo-17-swoosh";
import styles from "@/features/invoices/templates/invoice_fluvo_17/invoice-fluvo-17.module.css";
import { cn } from "@/lib/utils/cn";

/** Tailwind teal-800 — table header & total bar */
const ACCENT = "#115e59";
const ROW_TINT = "bg-teal-50/90";
...
const FIELD =
  "w-full min-w-0 border-0 border-b border-transparent bg-transparent py-0.5 text-sm text-zinc-800 placeholder:text-zinc-400 focus:border-teal-800 focus:outline-none";
const FIELD_RIGHT = `${FIELD} text-right tabular-nums`;

export function InvoiceFluvo17Template() {
      ...
      <div className="px-8 pb-10 pt-8 sm:px-10">
        ...
        <section className="mt-10 space-y-4">
              ...
              <tbody>
                {lines.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      <td className="border border-teal-200 p-2 align-top">
                        <input
                          className={FIELD}
                      ...
                      <td className="border border-teal-200 p-2 align-top text-center">
                        <input
                      ...
                      <td className="border border-teal-200 px-2 py-2 text-right align-top tabular-nums font-medium text-zinc-900">
                        {formatBdt(amt)}
                      </td>
                    </tr>
                  );
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={addLine}
              className="inline-flex items-center gap-1.5 rounded-lg border border-teal-400 bg-teal-50 px-3 py-1.5 text-xs font-medium text-teal-950 hover:bg-teal-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-800"
            >
              <Plus className="size-3.5 text-teal-800" aria-hidden />
              Add line
            </button>
            {lines.length > 1 ? (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-zinc-600 hover:text-teal-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-800"
              >
        ...
        <div className="mt-10 flex justify-end">
          <div className="w-full max-w-sm space-y-0 text-sm">
            <div className="flex justify-between gap-6 border-b border-teal-200 py-2.5">
              <span className="font-bold text-zinc-900">Subtotal</span>
              <span className="tabular-nums text-zinc-800">
                {formatBdt(subtotal)}
              </span>
            </div>
            <div className="flex items-center justify-between gap-6 border-b border-teal-200 py-2.5">
            ...
            <div className="flex items-center justify-between gap-6 border-b border-teal-200 py-2.5">
              <span className="font-bold text-zinc-900">
                Tax (
                <input
                  className="inline-block w-9 border-0 border-b border-transparent bg-transparent text-right font-bold focus:border-teal-800 focus:outline-none"
        ...
        <section className="mt-14 grid gap-12 lg:grid-cols-2 lg:items-start">
          <div className="flex min-w-0 flex-col gap-10">
            ...
            <footer className="border-t border-teal-200 pt-6 text-sm">
              <p className="font-bold text-zinc-900">Terms &amp; Conditions</p>
              <textarea
                className="mt-2 min-h-[5rem] w-full resize-y rounded-sm border-0 bg-transparent text-xs leading-relaxed text-zinc-600 focus:outline-none focus:ring-2 focus:ring-teal-800/20"
                value={terms}

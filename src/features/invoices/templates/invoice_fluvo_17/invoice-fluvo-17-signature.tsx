...
type InvoiceFluvo17SignatureProps = {
  className?: string;
...
export function InvoiceFluvo17Signature({
  ...
  return (
    <div className={cn("flex h-full flex-col gap-2", className)}>
      ...
      <div
        className={cn(
          "relative flex min-h-[160px] flex-1 items-center justify-center rounded-lg border border-dashed border-teal-600 bg-teal-50/80",
          imageUrl &&
      ...
      >
        {processing ? (
          <div className="flex flex-col items-center gap-2 py-8 text-sm text-zinc-600">
            <Loader2
              className="size-8 animate-spin text-teal-800"
              aria-hidden
            />
      ...
      <div className="flex flex-wrap gap-2">
        <label
          htmlFor={inputId}
          className={cn(
            "inline-flex cursor-pointer items-center gap-2 rounded-lg border border-teal-200 bg-white px-3 py-2 text-sm font-medium text-zinc-800 transition-colors hover:bg-teal-50 focus-within:ring-2 focus-within:ring-teal-800 focus-within:ring-offset-2",
            processing && "pointer-events-none opacity-50"
          )}
        >
          <ImageUp className="size-4 shrink-0 text-teal-800" aria-hidden />
        ...
        {imageUrl ? (
          <button
            ...
            className="rounded-lg px-3 py-2 text-sm font-medium text-zinc-600 underline-offset-2 hover:text-teal-900 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-800"
          >
            Remove
          </button>
        ) : null}

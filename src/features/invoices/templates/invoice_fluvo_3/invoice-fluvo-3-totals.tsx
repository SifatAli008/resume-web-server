...
const FIELD_RIGHT =
  "w-full min-w-0 border-0 border-b border-transparent bg-transparent py-0.5 text-sm text-zinc-800 focus:border-red-700 focus:outline-none text-right tabular-nums";

export type InvoiceFluvo3TotalsProps = {
  subtotal: number;
  discount: string;
...
export function InvoiceFluvo3Totals({
  subtotal,
  discount,
  invoiceTaxPercent,
  taxAmount,
    ...
    <div className="mt-10 flex justify-end">
      <div className="w-full max-w-sm space-y-0 text-sm">
        <div className="flex justify-between gap-6 border-b border-zinc-200 py-2.5">
          ...
          <span className="tabular-nums text-zinc-800">{formatBdt(subtotal)}</span>
        </div>
        ...
        <div className="flex items-center justify-between gap-6 border-b border-zinc-200 py-2.5">
          <span className="font-bold text-zinc-900">
            Tax (
            <input
              className="inline-block w-9 border-0 border-b border-transparent bg-transparent text-right font-bold focus:border-red-700 focus:outline-none"
          ...
          <span className="tabular-nums text-zinc-800">{formatBdt(taxAmount)}</span>
        </div>

        <div className="mt-2 flex justify-between gap-6 bg-red-800 px-4 py-4 text-lg font-bold text-white">
          <span>TOTAL</span>

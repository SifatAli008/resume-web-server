...
const FIELD =
  "w-full min-w-0 border-0 border-b border-transparent bg-transparent py-0.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-[#6b6ba3] focus:outline-none";
const FIELD_RIGHT = `${FIELD} text-right tabular-nums`;

const PURPLE = "#6b6ba3";

export type InvoiceFluvo8TableProps = {
  lines: InvoiceFluvo8LineRow[];
...
export function InvoiceFluvo8Table({
  lines,
  numericLines,
  onUpdateLine,
  onAddLine,
  onRemoveLastLine,
}: InvoiceFluvo8TableProps) {
  return (
    <section className="mt-12 space-y-4">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[540px] border-collapse border border-zinc-200 text-sm">
          <thead>
            <tr className="text-white" style={{ backgroundColor: PURPLE }}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          type="button"
          onClick={onAddLine}
          className="inline-flex items-center gap-1.5 rounded-lg border border-violet-200 bg-violet-50 px-3 py-1.5 text-xs font-medium text-zinc-900 hover:bg-violet-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6b6ba3]"
        >
          <Plus className="size-3.5" style={{ color: PURPLE }} aria-hidden />
          Add line
        </button>
        {lines.length > 1 ? (
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-zinc-600 hover:text-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6b6ba3]"
          >

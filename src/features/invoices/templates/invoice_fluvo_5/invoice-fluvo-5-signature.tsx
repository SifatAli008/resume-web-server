...
export function InvoiceFluvo5Signature({
  ...
  return (
    <div className={cn("relative z-[1] flex flex-col gap-3", className)}>
      ...
      <div
        className={cn(
          "relative flex min-h-[120px] items-center justify-center rounded-md border border-dashed border-blue-200 bg-blue-50/50",
          imageUrl &&
            "border-solid border-zinc-200 bg-[repeating-conic-gradient(#eff6ff_0%_25%,#ffffff_0%_50%)_50%/12px_12px]"
        )}
      >
        {processing ? (
      ...
      <div className="flex flex-wrap gap-2">
        <label
          htmlFor={inputId}
          className={cn(
            "inline-flex cursor-pointer items-center gap-2 rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm font-medium text-zinc-900 transition-colors hover:bg-blue-50 focus-within:ring-2 focus-within:ring-blue-600 focus-within:ring-offset-2",
            processing && "pointer-events-none opacity-50"
          )}
        >
          <ImageUp className="size-4 shrink-0 text-blue-600" aria-hidden />
        ...
        {imageUrl ? (
          <button
            ...
            className="rounded-lg px-3 py-2 text-sm font-medium text-zinc-600 underline-offset-2 hover:text-blue-800 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
          >
            Remove
          </button>
        ) : null}

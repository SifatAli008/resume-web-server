...
export type InvoiceFluvo7BillingInfoProps = {
  fromName: string;
  fromAddress: string;
  fromPhone: string;
  fromEmail: string;
...
export function InvoiceFluvo7BillingInfo({
  fromName,
  fromAddress,
  fromPhone,
  fromEmail,
  ...
  onBillPhoneChange,
  onBillEmailChange,
}: InvoiceFluvo7BillingInfoProps) {
  return (
    <section className="grid gap-10 sm:grid-cols-2">
      <div className="space-y-1.5 text-sm">
        <p className="text-xs font-bold uppercase tracking-wide text-[#00333d]">
          From
        </p>
        <input
      ...
      <div className="space-y-1.5 text-sm">
        <p className="text-xs font-bold text-[#00333d]">Bill To</p>
        <input
          className={cn(FIELD, "font-bold")}
          value={billName}

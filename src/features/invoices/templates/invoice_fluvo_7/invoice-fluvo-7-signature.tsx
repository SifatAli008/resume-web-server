...
export function InvoiceFluvo7Signature({
  ...
  return (
    <div className={cn("flex flex-col gap-3", className)}>
      <p className="text-sm font-bold text-[#00333d]">Signature</p>
      ...
      <div
        className={cn(
          "relative flex min-h-[120px] items-center justify-center rounded-md border border-dashed border-cyan-200 bg-cyan-50/40",
          imageUrl &&
            "border-solid border-zinc-200 bg-[repeating-conic-gradient(#ecfeff_0%_25%,#ffffff_0%_50%)_50%/12px_12px]"
        )}
      >
        {processing ? (
          <div className="flex flex-col items-center gap-2 py-6 text-sm text-zinc-600">
            <Loader2
              className="size-8 animate-spin text-cyan-600"
              aria-hidden
            />
      ...
      <div className="flex flex-wrap gap-2">
        <label
          htmlFor={inputId}
          className={cn(
            "inline-flex cursor-pointer items-center gap-2 rounded-lg border border-cyan-200 bg-white px-3 py-2 text-sm font-medium text-zinc-900 transition-colors hover:bg-cyan-50 focus-within:ring-2 focus-within:ring-cyan-500 focus-within:ring-offset-2",
            processing && "pointer-events-none opacity-50"
          )}
        >
          <ImageUp className="size-4 shrink-0 text-cyan-600" aria-hidden />
        ...
        {imageUrl ? (
          <button
            ...
            className="rounded-lg px-3 py-2 text-sm font-medium text-zinc-600 underline-offset-2 hover:text-[#00333d] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500"
          >
            Remove
          </button>
        ) : null}
